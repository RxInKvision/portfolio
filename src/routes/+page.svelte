<script>
	...
	function handleShapeClick(index, event) {
		...
		dragOffset = { x: event.clientX - rect.left - rect.width / 2, y: event.clientY - rect.top - rect.height / 2 };
	...
	function startAutoAnimation() { autoAnimationActive = true; }
	...
	function updatePatternPositions() {
		...
		if (svgLayer1) { svgLayer1.style.transform = `translate(${lastMouseX * 30}px, ${lastMouseY * 30 + smoothScrollY * 0.15}px) rotate(${smoothScrollY * 0.005}deg)`; }
		if (svgLayer2) { svgLayer2.style.transform = `translate(${lastMouseX * 25}px, ${lastMouseY * 25 + smoothScrollY * 0.1}px)`; }
		if (svgLayer3) { svgLayer3.style.transform = `translate(${lastMouseX * 15}px, ${lastMouseY * 15 + smoothScrollY * 0.08}px)`; }
		if (svgLayer4) { svgLayer4.style.transform = `translate(${lastMouseX * -20}px, ${lastMouseY * -20 - smoothScrollY * 0.1}px) rotate(${-smoothScrollY * 0.003}deg)`; }
		if (svgLayer5) { svgLayer5.style.transform = `translate(${lastMouseX * -25}px, ${lastMouseY * -25 - smoothScrollY * 0.12}px)`; }
...
<div class="landing-page" bind:this={pageContainer} class:light-mode={currentBackground === "yellow"} class:dark-mode={currentBackground === "black"}>
	<div class="floating-shapes-container">
		...
		<div class="floating-shape slanted-parallelogram shape-instance-5 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-6 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-7 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-8 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-9 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-10 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-11 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-12 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-13 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-14 reactive"></div>
		<div class="floating-shape slanted-parallelogram shape-instance-15 reactive"></div>
	</div>
	...
	<section class="works-preview" id="works-preview">
		...
		<div class="works-cards">
			<div class="work-card"><div class="card-inner"><div class="card-front"><div class="work-image image1"></div><div class="work-title"><h3>Interactive Installations</h3></div></div><div class="card-back"><p>Creative Direction for Installations</p><a href="/works" class="view-btn">View Work</a></div></div></div>
			<div class="work-card"><div class="card-inner"><div class="card-front"><div class="work-image image2"></div><div class="work-title"><h3>Live Visual Performance</h3></div></div><div class="card-back"><p>Real-time visual performance system for live music events</p><a href="/works" class="view-btn">View Work</a></div></div></div>
			<div class="work-card"><div class="card-inner"><div class="card-front"><div class="work-image image3"></div><div class="work-title"><h3>Brand Identity System</h3></div></div><div class="card-back"><p>Comprehensive visual identity for tech startup</p><a href="/works" class="view-btn">View Work</a></div></div></div>
		</div>
	</section>
...
<style>
	...
	.floating-shape {
		...
		transition: transform 0.6s cubic-bezier(0.19, 1, 0.22, 1), background 0.3s ease, box-shadow 0.3s ease;
	...
	.shape-instance-4 { bottom: 20%; right: 15%; }
	.shape-instance-5 { top: 50%; left: 40%; }
	.shape-instance-6 { top: 10%; right: 10%; }
	.shape-instance-7 { top: 70%; left: 8%; }
	.shape-instance-8 { top: 40%; right: 45%; }
	.shape-instance-9 { bottom: 15%; left: 35%; }
	.shape-instance-10 { top: 25%; left: 65%; }
	.shape-instance-11 { bottom: 30%; right: 30%; }
	...
	.svg-layer-1 { z-index: 1; opacity: 0.01; }
	.svg-layer-2 { z-index: 2; opacity: 0.015; }
	.svg-layer-3 { z-index: 3; opacity: 0.02; }
	.svg-layer-4 { z-index: 4; opacity: 0.015; }
	.svg-layer-5 { z-index: 5; opacity: 0.01; }
	.svg-pattern {
		position: absolute;
		width: 100%; height: 100%;
	...
	.giant-logotype-container.active { opacity: 1; transform: translateY(0); }
	.giant-logotype { width: 100%; height: auto; will-change: transform; }
	.title-container, .subtitle-container { opacity: 0; transform: translateY(20px); }
	.title-container { transition: opacity 1s ease 0.4s, transform 1s ease 0.4s; }
	.subtitle-container { transition: opacity 1s ease 0.6s, transform 1s ease 0.6s; }
	...
	.works-preview { min-height: 100vh; padding: 4rem 2rem; display: flex; flex-direction: column; align-items: center; position: relative; z-index: 2; }
	.section-title { font-size: 2.5rem; margin-bottom: 4rem; text-align: center; font-weight: bold; }
	.works-cards { display: flex; flex-wrap: wrap; justify-content: center; gap: 3rem; width: 100%; max-width: 1200px; }
	.work-card { width: 300px; height: 400px; perspective: 1000px; }
	.card-inner { position: relative; width: 100%; height: 100%; transition: transform 1s cubic-bezier(0.19, 1, 0.22, 1); transform-style: preserve-3d; }
	.work-card:hover .card-inner { transform: rotateY(180deg); }
	.card-front, .card-back { position: absolute; width: 100%; height: 100%; backface-visibility: hidden; background: rgba(0, 0, 0, 0.8); border: 1px solid var(--page-accent); display: flex; flex-direction: column; }
	...
	@media (max-width: 768px) {
		...
		.bg-far { background-size: 400px auto; }
		.bg-medium-far { background-size: 300px auto; }
		.bg-middle { background-size: 150px auto; }
		.bg-medium-near { background-size: 180px auto; }
		.bg-near { background-size: 70px auto; }
	}
	...
	@media (max-width: 480px) {
		...
		.works-preview { padding: 3rem 1rem; }
		.section-title { font-size: 2rem; margin-bottom: 2rem; }
		.work-card { width: 280px; height: 380px; }
